...
// GaleListOpts contains options for Gale.List
type GaleListOpts struct {
...
func (r *Gale) List(ctx context.Context, opts ...GaleListOpts) (string, error) {
	if r.list != nil {
		return *r.list, nil
	}
	q := r.q.Select("list")
	for i := len(opts) - 1; i >= 0; i-- {
		...
		// `workflowsDir` optional argument
		if !querybuilder.IsZeroValue(opts[i].WorkflowsDir) {
			q = q.Arg("workflowsDir", opts[i].WorkflowsDir)
		}
	}
...
// GaleRunOpts contains options for Gale.Run
type GaleRunOpts struct {
	// The directory containing the repository source. If source is provided, rest of the options are ignored.
	Source *Directory
	// The name of the repository. Format: owner/name.
	Repo string
...
func (r *Gale) Run(opts ...GaleRunOpts) *GaleWorkflowRun {
	q := r.q.Select("run")
	for i := len(opts) - 1; i >= 0; i-- {
		// `source` optional argument
		if !querybuilder.IsZeroValue(opts[i].Source) {
		...
		// `workflowFile` optional argument
		if !querybuilder.IsZeroValue(opts[i].WorkflowFile) {
			q = q.Arg("workflowFile", opts[i].WorkflowFile)
		}
		// `workflow` optional argument
		if !querybuilder.IsZeroValue(opts[i].Workflow) {
			q = q.Arg("workflow", opts[i].Workflow)
		}
		// `job` optional argument
		if !querybuilder.IsZeroValue(opts[i].Job) {
			q = q.Arg("job", opts[i].Job)
		}
		// `event` optional argument
		if !querybuilder.IsZeroValue(opts[i].Event) {
			q = q.Arg("event", opts[i].Event)
		}
		// `eventFile` optional argument
		if !querybuilder.IsZeroValue(opts[i].EventFile) {
			q = q.Arg("eventFile", opts[i].EventFile)
		}
		...
		// `runnerDebug` optional argument
		if !querybuilder.IsZeroValue(opts[i].RunnerDebug) {
			q = q.Arg("runnerDebug", opts[i].RunnerDebug)
		}
...
type GaleRunner struct {
	q *querybuilder.Selection
	c graphql.Client
}

// GaleRunnerContainerOpts contains options for GaleRunner.Container
type GaleRunnerContainerOpts struct {
...
func (r *GaleRunner) Container(opts ...GaleRunnerContainerOpts) *GaleRunnerContainer {
	...
	return &GaleRunnerContainer{
		q: q,
		c: r.c,
	}
}

type GaleRunnerContainer struct {
	q *querybuilder.Selection
	c graphql.Client
...
// Container initializes a new runner container with the given options.
func (r *GaleRunnerContainer) Container() *Container {
	q := r.q.Select("container")
...
func (r *GaleRunnerContainer) Ctr() *Container {
	q := r.q.Select("ctr")
...
func (r *GaleRunnerContainer) RunnerID(ctx context.Context) (string, error) {
	if r.runnerId != nil {
		return *r.runnerId, nil
	}
	q := r.q.Select("runnerId")
...
type GaleWorkflowRun struct {
	q *querybuilder.Selection
	c graphql.Client
}

// Configuration of the workflow run.
func (r *GaleWorkflowRun) Config() *GaleWorkflowRunConfig {
	q := r.q.Select("config")

	return &GaleWorkflowRunConfig{
		q: q,
		c: r.c,
	}
}

// GaleWorkflowRunDirectoryOpts contains options for GaleWorkflowRun.Directory
type GaleWorkflowRunDirectoryOpts struct {
...
func (r *GaleWorkflowRun) Directory(opts ...GaleWorkflowRunDirectoryOpts) *Directory {
	q := r.q.Select("directory")
	for i := len(opts) - 1; i >= 0; i-- {
		// `includeRepo` optional argument
		if !querybuilder.IsZeroValue(opts[i].IncludeRepo) {
			q = q.Arg("includeRepo", opts[i].IncludeRepo)
		}
		// `includeSecrets` optional argument
		if !querybuilder.IsZeroValue(opts[i].IncludeSecrets) {
			q = q.Arg("includeSecrets", opts[i].IncludeSecrets)
		}
		// `includeEvent` optional argument
		if !querybuilder.IsZeroValue(opts[i].IncludeEvent) {
			q = q.Arg("includeEvent", opts[i].IncludeEvent)
		}
		// `includeArtifacts` optional argument
		if !querybuilder.IsZeroValue(opts[i].IncludeArtifacts) {
			q = q.Arg("includeArtifacts", opts[i].IncludeArtifacts)
		}
	}
...
func (r *GaleWorkflowRun) Runner() *GaleRunnerContainer {
	q := r.q.Select("runner")
...
func (r *GaleWorkflowRun) Sync() *Container {
	q := r.q.Select("sync")
...
type GaleWorkflowRunConfig struct {
	q *querybuilder.Selection
	c graphql.Client
...
// Name of the event that triggered the workflow. e.g. push
func (r *GaleWorkflowRunConfig) Event(ctx context.Context) (string, error) {
	if r.event != nil {
		return *r.event, nil
	}
	q := r.q.Select("event")
...
// File with the complete webhook event payload.
func (r *GaleWorkflowRunConfig) EventFile() *File {
	q := r.q.Select("eventFile")
...
func (r *GaleWorkflowRunConfig) Job(ctx context.Context) (string, error) {
	if r.job != nil {
		return *r.job, nil
	}
	q := r.q.Select("job")
...
// Enables debug mode.
func (r *GaleWorkflowRunConfig) RunnerDebug(ctx context.Context) (bool, error) {
	if r.runnerDebug != nil {
		return *r.runnerDebug, nil
	}
	q := r.q.Select("runnerDebug")
...
// GitHub token to use for authentication.
func (r *GaleWorkflowRunConfig) Token() *Secret {
	q := r.q.Select("token")
...
// Workflow to run.
func (r *GaleWorkflowRunConfig) Workflow(ctx context.Context) (string, error) {
	if r.workflow != nil {
		return *r.workflow, nil
	}
	q := r.q.Select("workflow")
...
func (r *GaleWorkflowRunConfig) WorkflowFile() *File {
	q := r.q.Select("workflowFile")
...
func (r *GaleWorkflowRunConfig) WorkflowsDir(ctx context.Context) (string, error) {
	if r.workflowsDir != nil {
		return *r.workflowsDir, nil
	}
	q := r.q.Select("workflowsDir")
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (any, error) {
	...
	case "Ptime":
		...
		case "Test":
			...
			return nil, (*Ptime).Test(&parent, ctx)
		case "Puilt":
			var err error
			var parent Ptime
			err = json.Unmarshal(parentJSON, &parent)
			...
			return nil, (*Ptime).Puilt(&parent, ctx)
		case "Lint":
			var err error
			var parent Ptime
			err = json.Unmarshal(parentJSON, &parent)
			...
			return nil, (*Ptime).Lint(&parent, ctx)
		case "Build":
			var err error
			var parent Ptime
			err = json.Unmarshal(parentJSON, &parent)
			...
			return nil, (*Ptime).Build(&parent, ctx)
		case "RubocopReviewdog":
			var err error
			var parent Ptime
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return nil, (*Ptime).RubocopReviewdog(&parent, ctx)
		default:
